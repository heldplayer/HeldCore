...
public class SGTextField extends SGInteractable {
    ...
    public void setSelectionPosition(int position) {
        ...
        String visible = this.font.trimStringToWidth(this.value.substring(this.scrollOffset), width);
        int offset = visible.length() + this.scrollOffset;
        if (position == this.scrollOffset) {
            this.scrollOffset = this.font.trimStringToWidth(this.value, width, true).length();
        }
    ...
    public void onClick(int mouseX, int mouseY, int button) {
        ...
        if (this.hasFocus() && button == 0) {
            String trimmed = this.font.trimStringToWidth(this.value.substring(this.scrollOffset), this.getWidth(SizeContext.INNER));
            if (GuiScreen.isShiftKeyDown()) {
